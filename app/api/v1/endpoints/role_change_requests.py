...
async def get_role_change_requests(
    *,
    ...
    # 自分が作成したリクエストを取得
    my_requests = await crud_role_change_request.get_by_requester(
        db=db,
        requester_staff_id=current_user.id
    )
    print(f"[DEBUG ROLE_CHANGE_REQUEST] My requests count: {len(my_requests)}")
    for req in my_requests:
        print(f"[DEBUG ROLE_CHANGE_REQUEST]   - Request {req.id}: {req.from_role} → {req.requested_role}, status={req.status}")
    ...
    if current_user.role in [StaffRole.manager, StaffRole.owner]:
        if current_user.office_associations:
            office_id = current_user.office_associations[0].office_id
            approvable_requests = await crud_role_change_request.get_pending_for_approver(
                db=db,
...
async def approve_role_change_request(
    *,
    ...
    """
    # リクエストを取得
    request = await crud_role_change_request.get(db=db, id=request_id)
    if not request:
        raise HTTPException(
