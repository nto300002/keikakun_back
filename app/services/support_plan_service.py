    ...
    async def handle_deliverable_upload(
        ...
        *,
        ...
        if latest_status.step_type != target_step_type:
            from app.core.exceptions import InvalidStepOrderError
            logger.error("Invalid deliverable step order")
        ...
        if target_step_type == SupportPlanStep.final_plan_signed:
            try:
                await calendar_service.delete_event_by_cycle(
                    db=db,
                    cycle_id=cycle.id,
        ...
        if target_step_type == SupportPlanStep.monitoring:
            try:
                await calendar_service.delete_event_by_status(
                    db=db,
        ...
        try:
            ...
            logger.error("Failed to commit deliverable upload: %s", type(commit_error).__name__)
            raise
