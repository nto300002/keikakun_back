...
class WelfareRecipientService:
    ...
    async def create_recipient_with_initial_plan(
        ...
        try:
            ...
            await WelfareRecipientService._create_initial_support_plan(db, welfare_recipient.id)

            # 5. すべての変更をフラッシュしてDBに反映準備
            await db.flush()
    
            # 6. オブジェクトのIDを返す (コミットは呼び出し元で行う)
            return welfare_recipient.id
    ...
    @staticmethod
    async def _create_initial_support_plan(db: AsyncSession, welfare_recipient_id: UUID) -> None:
        """
