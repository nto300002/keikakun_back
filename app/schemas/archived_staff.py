...
class ArchivedStaffBase(BaseModel):
    ...
    archive_reason: str = Field(..., description="保管理由")
    legal_retention_until: datetime = Field(..., description="法定保存期限")


class ArchivedStaffRead(ArchivedStaffBase):
    """保管済みスタッフ読み取り用スキーマ（詳細）"""
    id: UUID = Field(..., description="保管情報ID")
    archived_at: datetime = Field(..., description="保管日時")
...
class ArchivedStaffListResponse(BaseModel):
    """保管済みスタッフリストレスポンス"""
    items: List[ArchivedStaffListItem] = Field(..., description="保管済みスタッフ一覧")
