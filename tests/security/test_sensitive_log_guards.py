...
def test_totp_values_are_not_logged():
    ...
    mfa_source = read_app_source("app/services/mfa.py")
    staff_source = read_app_source("app/models/staff.py")
    log_lines = [
        line
        for line in (auth_source + "\n" + security_source + "\n" + mfa_source + "\n" + staff_source).splitlines()
        if "logger." in line
    ]
...
def test_raw_request_data_tracebacks_and_storage_keys_are_not_logged_in_sensitive_paths():
    checked_paths = [
    ...
    for relative_path in checked_paths:
        ...
        log_lines = "\n".join(line for line in source.splitlines() if "logger." in line)
        for fragment in forbidden_fragments:
            assert fragment not in log_lines
