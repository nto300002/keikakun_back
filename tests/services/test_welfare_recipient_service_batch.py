...
@pytest.mark.asyncio
async def test_get_deadline_alerts_does_not_eager_load_deliverables_for_assessment_alerts(
    db_session: AsyncSession,
    test_offices_with_data: dict,
    monkeypatch: pytest.MonkeyPatch
):
    """
    【性能回帰テスト】アセスメント未完了判定で成果物一覧を全件 eager load しない。

    PlanDeliverable は NOT EXISTS で存在確認し、SupportPlanCycle.deliverables の
    selectinload に戻るとこのテストが失敗する。
    """
    original_selectinload = sqlalchemy_orm.selectinload

    def guarded_selectinload(attr, *args, **kwargs):
        if getattr(attr, "key", None) == "deliverables":
            raise AssertionError("deadline alerts must not eager load cycle.deliverables")
        return original_selectinload(attr, *args, **kwargs)

    monkeypatch.setattr(sqlalchemy_orm, "selectinload", guarded_selectinload)

    alerts = await WelfareRecipientService.get_deadline_alerts(
        db=db_session,
        office_id=test_offices_with_data["office_ids"][0],
        threshold_days=30,
        limit=10,
        offset=0
    )
    ...
    assert any(alert.alert_type == "assessment_incomplete" for alert in alerts.alerts)


@pytest.mark.asyncio
async def test_get_deadline_alerts_batch_does_not_eager_load_deliverables_for_assessment_alerts(
    db_session: AsyncSession,
    test_offices_with_data: dict,
    monkeypatch: pytest.MonkeyPatch
):
    """
    【性能回帰テスト】バッチ期限アラートでも成果物一覧を全件 eager load しない。
    """
    original_selectinload = sqlalchemy_orm.selectinload

    def guarded_selectinload(attr, *args, **kwargs):
        if getattr(attr, "key", None) == "deliverables":
            raise AssertionError("batch deadline alerts must not eager load cycle.deliverables")
        return original_selectinload(attr, *args, **kwargs)
    ...
    assert len(alerts_by_office) == len(test_offices_with_data["office_ids"])
    for alerts in alerts_by_office.values():
        assert any(alert.alert_type == "assessment_incomplete" for alert in alerts.alerts)


@pytest.mark.asyncio
async def test_get_deadline_alerts_applies_limit_offset_in_sql(
    db_session: AsyncSession,
    test_offices_with_data: dict,
    monkeypatch: pytest.MonkeyPatch
):
    """
    【性能回帰テスト】期限アラートのページングは Python slicing ではなく SQL で行う。
    """
    ...
    async def spy_execute(statement, *args, **kwargs):
        try:
            executed_sql.append(
                str(statement.compile(compile_kwargs={"literal_binds": False})).lower()
            )
        except Exception:
            pass
    ...
    assert any(" limit " in sql and " offset " in sql for sql in executed_sql), (
        "deadline alerts must push limit/offset down to SQL"
    )
